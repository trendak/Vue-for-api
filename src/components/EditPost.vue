...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      savePost() {
        this.$http.post('http://localhost/Rest_API/api/post/update.php?id=' +this.post.id, this.post)
          .then(() => {
      ...
      getPost(id){
        this.$http.get('http://localhost/Rest_API/api/post/read_single.php?id='+id)
          .then(respone => respone.json())
